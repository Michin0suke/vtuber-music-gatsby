...
export default ({ artist, href, className, hoverEffect }) => {
  ...
  return (
        <div className={`relative bg-white rounded-full shadow overflow-hidden ${className}`}>
            ...
            {href &&
            <a href={href} target='_blank' className='absolute top-0 left-0 w-full h-full bg-white opacity-0 sm:hover:opacity-20 text-xs flex items-center justify-center whitespace-pre' rel="noreferrer">{href}</a>}
        </div>
  )
}
