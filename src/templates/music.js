...
export default ({ data: { music } }) => {
  const [frameColor] = useState(Please.make_color({ format: 'rgb-string', saturation: 0.2 }))
...
<div className='w-full'>
    <SEO
        ...
        imgUrl={music.videos?.[Math.floor(Math.random() * music.videos.length)]?.thumbnail_image?.childImageSharp?.fluid?.src}
        isLargeCard
    />

    <div className='max-w-4xl mx-auto'>
        <div className={'mb-7 pb-3 bg-white lg:shadow'}>
            {/* eslint-disable-next-line  jsx-a11y/click-events-have-key-events */}{/* eslint-disable-next-line  jsx-a11y/no-static-element-interactions */}{/* eslint-disable-next-line jsx-a11y/aria-role */}
            <div role='' className={`w-4/5 max-w-md mx-auto pt-2 photo-frame ${isPhotoFrameMoving && 'moving'}`} onClick={() => isPhotoFrameMoving || movePhotoFrame()}>
